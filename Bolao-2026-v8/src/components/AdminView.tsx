...
export const AdminView: React.FC<AdminViewProps> = ({
  ...
  const [allSupabaseBets, setAllSupabaseBets] = useState<any[]>([]);
  ...
  // Load all bets once for Supabase mode (to compute points preview locally)
  useEffect(() => {
    if (!isDBConnected) return;
    let cancelled = false;
    getSupabaseAllBets().then(bets => { if (!cancelled) setAllSupabaseBets(bets); });
    return () => { cancelled = true; };
  }, [isDBConnected]);

  // Compute live points preview when admin types scores
  const pointsPreview = useMemo(() => {
    const previews: Record<string, { items: any[]; distribution: number[]; total: number; avg: number }> = {};

    const bets = isDBConnected
      ? allSupabaseBets.map((b: any) => ({ ...b, predA: b.bet_goals_a, predB: b.bet_goals_b }))
      : getStoredPredictions().map((p: any) => ({ ...p, predA: p.placar_time_a, predB: p.placar_time_b }));

    if (bets.length === 0) return previews;

    for (const entry of Object.entries(resultForm)) {
      const [matchId, form] = entry as [string, { goalsA: string; goalsB: string; status: string }];
      if (!form.goalsA || !form.goalsB) continue;
      const gA = parseInt(form.goalsA, 10);
      const gB = parseInt(form.goalsB, 10);
      if (isNaN(gA) || isNaN(gB)) continue;

      const matchBets = bets.filter((b: any) => b.match_id === matchId);
      if (matchBets.length === 0) continue;

      const items = matchBets.map((b: any) => {
        const calc: any = isDBConnected
          ? calculatePoints(gA, gB, b.predA, b.predB)
          : calculatePredictionPoints(gA, gB, b.predA, b.predB);
        return {
          ...calc,
          points: calc.points_total ?? calc.points,
          predA: b.predA,
          predB: b.predB,
        };
      });
      ...
      let totalPoints = 0;
      items.forEach((i: any) => {
        distribution[i.points] = (distribution[i.points] || 0) + 1;
      ...
      previews[matchId] = {
        ...
        avg: matchBets.length > 0 ? totalPoints / matchBets.length : 0,
      };
    }

    return previews;
  }, [resultForm, allSupabaseBets, isDBConnected]);
  ...
  const startEditing = (match: Match) => {
    ...
    setEditForm({
      ...
      flag_a: match.teamA.logo,
      flag_b: match.teamB.logo,
  ...
  const handleSaveEdit = async (matchId: string) => {
    ...
    try {
      const data = { ...editForm };
      if (data.stadium && data.city) {
        data.stadium = `${data.stadium}, ${data.city}`;
      }
      delete data.city;

      if (isDBConnected) {
        const res = await updateSupabaseMatch(matchId, data);
        if (!res.success) throw new Error(res.message);
      } else {
        const stored = getStoredMatches();
        const updated = stored.map((m: any) =>
  ...
  const handleDeleteMatch = async (matchId: string) => {
    ...
    try {
      if (isDBConnected) {
        const res = await deleteSupabaseMatch(matchId);
        if (!res.success) throw new Error(res.message);
      } else {
        const stored = getStoredMatches();
        saveStoredMatches(stored.filter((m: any) => m.id !== matchId));
        recalculateEveryonePoints(getStoredMatches());
      }
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-black/40 border-b border-white/10 text-[10px] font-black uppercase tracking-wider text-primary">
                ...
                <th className="py-3 px-3 w-28">Gerenciar</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={match.id} className="hover:bg-white/5 transition-all">
                    ...
                    <td className="py-2.5 px-3">
                      ...
                      {pointsPreview[match.id] && (
                        <div className="mt-1.5 text-[9px] text-center leading-tight">
                          <span className="text-green-400 font-bold">
                            {pointsPreview[match.id].total} palpites
                          </span>
                          <span className="text-on-surface-variant">
                            {' '}| Média{' '}
                          </span>
                          <span className="text-amber-400 font-bold">
                            {pointsPreview[match.id].avg.toFixed(1)}pts
                          </span>
                          <div className="text-on-surface-variant mt-0.5">
                            {pointsPreview[match.id].distribution.map((count: number, pt: number) =>
                              count > 0 ? (
                                <span key={pt} className="mr-1">
                                  <span className={pt === 5 ? 'text-yellow-400' : pt >= 3 ? 'text-green-400' : 'text-on-surface-variant'}>
                                    {pt}pts
                                  </span>
                    ...
                    <td className="py-2.5 px-3">
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="p-1.5 bg-[#1670D8]/20 hover:bg-[#1670D8]/40 text-[#1670D8] rounded-lg transition-all cursor-pointer"
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={cancelEditing} className="p-1 hover:bg-white/10 rounded-lg transition-all cursor-pointer">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditForm(p => ({ ...p, team_a: e.target.value }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditForm(p => ({ ...p, team_b: e.target.value }))}
                    className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-xs text-on-surface outline-none focus:border-primary"
                  />
                </div>
                <div>
                  ...
                  <input
                    value={editForm.flag_a || ''}
                    onChange={e => setEditForm(p => ({ ...p, flag_a: e.target.value }))}
                    className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-xs text-on-surface outline-none focus:border-primary"
                  />
                </div>
                <div>
                  ...
                  <input
                    value={editForm.flag_b || ''}
                    onChange={e => setEditForm(p => ({ ...p, flag_b: e.target.value }))}
                    className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-xs text-on-surface outline-none focus:border-primary"
                  />
                </div>
                <div className="col-span-2">
                  ...
                  <input
                    type="datetime-local"
                    value={editForm.match_date ? editForm.match_date.substring(0, 16) : ''}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditForm(p => ({ ...p, stadium: e.target.value }))}
                    className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-xs text-on-surface outline-none focus:border-primary"
                  />
                </div>
                <div>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button
                  onClick={() => handleSaveEdit(editingMatchId)}
                  disabled={savingEdit}
                  className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/80 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all disabled:opacity-50 cursor-pointer"
                >
                  <Save size={14} />
        ...
        {deleteConfirmId && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.9, opacity: 0 }}
              className="bg-surface-container border border-white/10 rounded-2xl p-6 w-full max-w-sm shadow-2xl text-center"
              onClick={e => e.stopPropagation()}
            >
              <AlertTriangle size={40} className="text-red-400 mx-auto mb-4" />
              <h3 className="font-headline text-sm font-black text-on-surface uppercase tracking-wide mb-2">
                Excluir Partida?
              </h3>
              <p className="text-xs text-on-surface-variant mb-6">
                Esta ação irá remover a partida e todos os palpites associados. Esta operação não pode ser desfeita.
              </p>
              <div className="flex justify-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all disabled:opacity-50 cursor-pointer"
                >
                  <Trash2 size={14} />
